...
*
* $Id: Savant.php,v 1.1.1.1 2004/09/20 21:42:34 silence Exp $
* 
*/
...
class Savant {
	...
	*/
	...
	{	
		...
		//
		trigger_error($this->_errors[SAVANT_ERROR_ASSIGN], E_USER_WARNING);
	}
	...
	*/
	...
	function assignRef($name, &$ref)
	{
		...
		// final error catch
		trigger_error($this->_errors[SAVANT_ERROR_ASSIGN_REF], E_USER_WARNING);
	}
	...
	*/
	...
	function assignObject($name, &$obj)
	{
		...
		if (! is_object($obj)) {
			...
			trigger_error($this->_errors[SAVANT_ERROR_ASSIGN_NOT_OBJECT], E_USER_WARNING);
		}
		
		// final error catch
		trigger_error($this->_errors[SAVANT_ERROR_ASSIGN_OBJECT], E_USER_WARNING);
	}
	...
	*/
	...
	function findTemplate($tpl)
	{
		...
		if (! $file) {
			...
			trigger_error($this->_errors[SAVANT_ERROR_TEMPLATE_NOT_FOUND], E_USER_WARNING);
		} else {
			return $file;
		}
	}
	...
	* @access public
	*
	* @param string $tpl The template name to look for.
	*
	...
	*/
	...
	function _getTemplate()
	{
		...
		if ($k > 0) {
			...
			trigger_error($this->_errors[SAVANT_ERROR_TEMPLATE_STACK_EMPTY], E_USER_WARNING);
		}
	}
	...
	*/
	...
	function _loadPlugin($requested_name)
	{
		...
		if (! $file) {
			...
			trigger_error($this->_errors[SAVANT_ERROR_PLUGIN_NOT_FOUND], E_USER_WARNING);
		} else {
	...
	*/
	...
	function _loadFilter($requested_name)
	{
		...
		if (! $file) {
			...
			trigger_error($this->_errors[SAVANT_ERROR_FILTER_NOT_FOUND], E_USER_WARNING);
		} else {
	...
	*/
	...
	function getTokens($key = null)
	{
		if (is_null($key)) {
			...
			trigger_error($this->_errors[SAVANT_ERROR_TOKEN_NOT_FOUND], E_USER_WARNING);
		}
	}
